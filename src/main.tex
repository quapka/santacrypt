...
% The layout of general APDU command and response is in table~\ref{tab:apdu}.
...
\section{JavaCard Vulnerability Scanner}
    The work done in~\cite{Kvapil2020thesis} resulted in an automated testing tool called \projectname. The thesis explains its architecture, design choices and usage in much more detail. Here we will only present the most important points.
...
% \subsection{The framework specifications}
    ...
    The framework uses Docker to achieve cross-platformity or can be invoked natively on Linux. The results of the individual runs can be viewed in a simple web application (see figure~\ref{fig:web-interface-all}).
...
In this section we will utilize the attacks discussed in the section~\ref{chp:state-of-the-art} and the testing framework JavaCard Vulnerability Scanner presented in the previous section and analyze a set of real JavaCards. Aquiring a large number of distinct JavaCards is not an easy task (as they are usually hard to come by in small $< 1000$ quantities), however, thanks to the joined work of the members of CRoCS laboratory~\cite{crocsweb} we had an interesting variety of JavaCards available for testing. The tested JavaCard were fabricated between the years 2011 a 2017 and the operating systems on these cards were released between the years 2010 and 2018. The list of the cards can be seen in table ~\ref{tab:card-list}.
